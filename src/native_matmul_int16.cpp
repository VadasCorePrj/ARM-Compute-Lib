...
void fillTensor(int16_t** src, int row, int col)
{
	...
	for(int y = 0; y < row; y++)
	{
		src[y] = (int16_t*)malloc(sizeof(int16_t)*col);	
		for(int x = 0; x < col; x++)
		{
			src[y][x] = (int16_t)(((static_cast<float>(rand()) / static_cast<float>(RAND_MAX/2)) - 1) * 10);
		}
	}
}

void freeTensor(int16_t** src, int row, int col)
{
	for(int y = 0; y < row; y++)
	{
		free(src[y]);
...
void printTensor(int16_t** src, const int row, const int col)
{
	for(int y = 0; y < row; y++)
	{
		int16_t* tmp = src[y];
...
int main(int argc, char** argv)
{
	...
	src0 = (int16_t**)malloc(sizeof(int16_t*)*N);
	src1 = (int16_t**)malloc(sizeof(int16_t*)*K);
	dst = (int16_t**)malloc(sizeof(int16_t*)*N);
